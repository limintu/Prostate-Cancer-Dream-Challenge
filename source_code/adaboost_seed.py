...
class Adaboost:
    ...
    def __init__(self, regr_name, random_fname):
    ...
    def fit(self, X, y, delta, n_iter, sample_weight=None):
        ...
        for i in xrange(n_iter):
            ...
            while regr is None or sample_weight is None or regr_weight is None:
                ...
                #print 'count of error_M > error_M_uc = {0}'.format(count)
    ...
    def predict(self, X, mode="max"):
        ...
        if mode == 'max':
            ...
            print 'weight_list = {0}'.format(weight_list)
            m = max(weight_list)
            max_i_list = [i for i, j in enumerate(weight_list) if j == m]
            print 'max_i_list = {0}'.format(max_i_list)
	...
	#This part is changed by Yofu
        ...
        #print 'sample_weight = {0} before normalization'.format(sample_weight)
        sample_weight = self._normalizing_weight(sample_weight)
        #print 'sample_weight = {0} after normalization'.format(sample_weight)
	...
	print 'sample_weight = {0}'.format(sample_weight)
        ...
        X = np.array([X[i] for i in i_array.tolist()])
        y = np.array([y[i] for i in i_array.tolist()])
        ### written by Yufu
        with open(self.random_fname, 'a+') as f:
            f.write(" ".join(map(str, i_array)))
            f.write("\n")
        ### 
        delta = np.array([delta[i] for i in i_array.tolist()])
        sample_weight = np.array([sample_weight[i] for i in i_array.tolist()])
        sample_weight = self._normalizing_weight(sample_weight)
        return X, y, delta, sample_weight
